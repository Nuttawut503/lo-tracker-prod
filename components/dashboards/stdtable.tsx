...
export function IndividualPLO(props: { studentID: string }) {
  ...
  useEffect(() => {
    calculatePLO()
  }, [dashboardFlat, calculatePLO]) //probably work
  ...
  }, [dataType, compareID, studentLOHead, studentLOScore, studentPLOHead, studentPLOScore, totalLOScore, totalPLOScore])
  ...
  }, [compareID, dataType, tableData, totalLOScore, totalPLOScore])
      ...
      <div>
        <div style={{ display: "inline-block" }}>
          ...
          <select value={compareID} onChange={handleCompare} className="border rounded-md border-2 ">
            <option value="---">---</option>
            {allData.map(std => (
              <option key={std.studentID} value={std.studentID}>{std.studentID}</option>
            ))}
          </select>
        </div>
        <br />
...
export function IndividualQuiz (props: { studentID: string }) {
  ...
  }, [dashboardQuiz, students, displayScore]) //probably work
